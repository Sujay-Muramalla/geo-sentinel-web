document.addEventListener("DOMContentLoaded", () => {
    ...
    const sentimentFilterInputs = Array.from(
        document.querySelectorAll('input[name="sentiment-filter"]')
    );
    ...
    function getSelectedCountries() {
        ...
        return Array.from(selectedCountriesContainer.querySelectorAll(".country-chip"))
            .map((chip) =>
                normalizeText(chip.dataset.country || chip.textContent.replace("×", ""))
            )
            .filter(Boolean);
    }
    ...
    function syncStateFromUI() {
        ...
        const sentimentChoice = document.querySelector(
            'input[name="sentiment-filter"]:checked'
        );
        state.sentimentFilter = sentimentChoice ? sentimentChoice.value : "all";

        state.sortBy = sortResultsSelect ? sortResultsSelect.value : "final-desc";
    }
    ...
    function getCardSearchText(card) {
        ...
        const title =
            card.dataset.title || card.querySelector(".result-title")?.textContent || "";
        const description =
            card.dataset.description ||
            card.querySelector(".result-description")?.textContent ||
            "";
    ...
    function calculateQueryRelevance(card, query) {
        ...
        if (!tokens.length) {
            const backendFinal = parseFloat(card.dataset.finalScore || "0");
            const backendSignal = parseFloat(card.dataset.signalScore || card.dataset.signal || "0");

            return {
                isMatch: true,
                rawScore: backendFinal || backendSignal,
                normalizedScore: backendFinal || backendSignal,
                matchedTokens: [],
                totalTokens: 0
            };
        }
        ...
        tokens.forEach((token) => {
            const tokenMatched = tokenMatchesSearchableText(
                token,
                searchableText,
                searchableTokens
            );
        ...
        const sourceText = normalizeText(
            card.dataset.source || card.querySelector(".result-source")?.textContent || ""
        );
        ...
        const backendFinal = parseFloat(card.dataset.finalScore || "0");
        const backendSignal = parseFloat(card.dataset.signalScore || card.dataset.signal || "0");
        const normalizedScore = backendFinal > 0 ? backendFinal * 100 : Math.max(computedScore, backendSignal);
    ...
    function updateCardTextHighlights(card, query) {
        ...
        const baseDescription =
            card.dataset.description || descriptionEl?.textContent || "";
    ...
    function updateCardDisplays(card, displayScore, sentimentScore, query) {
        ...
        if (signalValueEl) {
            signalValueEl.textContent = `${Math.round(displayScore)}/100`;
        }
        ...
        if (sentimentTagEl) {
            ...
            sentimentTagEl.classList.remove(
                "sentiment-positive",
                "sentiment-neutral",
                "sentiment-negative"
            );
    ...
    function sortVisibleEntries(entries) {
        ...
        entries.sort((a, b) => {
            if (sortMode === "final-desc") return b.finalScore - a.finalScore || a.originalIndex - b.originalIndex;
            ...
            return b.finalScore - a.finalScore || a.originalIndex - b.originalIndex;
        });
    ...
    function updateResultsToolbar(sortedEntries) {
        ...
        if (activeSortLabel && sortResultsSelect) {
            const selectedText =
                sortResultsSelect.options[sortResultsSelect.selectedIndex]?.text ||
                "Intelligence Score";
        ...
        if (dominantSentimentLabel) {
            if (!sortedEntries.length) {
                ...
                if (negative > positive && negative > neutral) {
                    dominantSentimentLabel.textContent = "Negative";
                } else if (positive > negative && positive > neutral) {
                    dominantSentimentLabel.textContent = "Positive";
                } else {
                    dominantSentimentLabel.textContent = "Mixed";
                }
            }
        }
    }
    ...
    function updateSignalMetrics(sortedEntries) {
        ...
        const avgSignal =
            sortedEntries.reduce((sum, entry) => sum + entry.signalScore, 0) /
    ...
    function buildResultCardMarkup(item) {
        ...
        const publishedLabel = formatPublishedDate(item.publishedAt || item.date);
        const displayScore =
        ...
        card.dataset.signal = String(displayScore);
        card.dataset.finalScore = String(item.finalScore || 0);
        card.dataset.published = item.publishedAt || item.date || "";
        card.dataset.url = item.url || "";

        const chips = [
            `<span class="result-chip">${escapeHtml(titleCase(region.replace(/-/g, " ")))}</span>`,
            item.country
                ? `<span class="result-chip">${escapeHtml(titleCase(item.country))}</span>`
                : "",
            item.mediaType
                ? `<span class="result-chip">${escapeHtml(item.mediaType)}</span>`
                : ""
        ]
            .filter(Boolean)
            .join("");
    ...
    function filterAndRenderResults() {
        ...
        resultCards.forEach((card, index) => {
            ...
            const finalScore = parseFloat(card.dataset.finalScore || "0");
            const displayScore =
                finalScore > 0 ? finalScore * 100 : queryRelevance.normalizedScore || signalScore;

            updateCardDisplays(card, displayScore, sentimentScore, state.query);
    ...
    async function fetchIntelligenceResults() {
        ...
        try {
            ...
            state.lastSummary = {
                totalSources: data?.metrics?.totalResults || 0,
                averageSentiment: data?.metrics?.averageSentimentScore || 0,
                averageSignal: data?.metrics?.averageSignalScore || 0,
                averageFinal: data?.metrics?.averageFinalScore || 0
            };
    ...
    function clearAllFilters() {
        ...
        if (sortResultsSelect) {
            sortResultsSelect.value = "final-desc";
        }
    ...
    function initializeCardDisplays() {
        getResultCards().forEach((card) => {
            ...
            const signalScore = parseFloat(card.dataset.signalScore || "0");
            const sentimentScore = parseFloat(card.dataset.sentimentScore || "0");
            const displayScore = finalScore > 0 ? finalScore * 100 : signalScore;

            updateCardDisplays(card, displayScore, sentimentScore, "");
        });
    }
    ...
    async function init() {
        ...
        if (sortResultsSelect && !sortResultsSelect.value) {
            sortResultsSelect.value = "final-desc";
        }
